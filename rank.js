  ...
  this.resetRank = function (data, force) {
    ...
    if (force || confirm('昇階リセットして、階位' + gainrank + 'を得ますか？')) {
      ...
      if (data.player.onchallenge) {
        ...
        if (data.player.challengecleared.length >= 128 && !data.player.rankchallengecleared.includes(data.challengedata.calcchallengeid(data))) {
          data.player.rankchallengecleared.push(data.challengedata.calcchallengeid(data))
        }
      }
